...
class ResnetPGDAttacker:
    ...
    def pgd_attack(self, image, label, eps=None, alpha=None, steps=None):
        ...
        # Starting at a uniformly random point within eps ball (around the original image, instead of starting at the original image)
        # The purpose is to increase the diversity of attacks and to some extent avoid falling into local optima
        ...
        for _ in range(steps):
            ...
            adv_images = adv_images.detach() # Detach adv_images from the computation graph to avoid accumulating gradients
            # Gradient update
